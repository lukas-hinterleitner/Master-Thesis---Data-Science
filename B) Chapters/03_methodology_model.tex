\chapter{Gradient Similarity as a Method for Explanation}
...
\For{$t \gets 1$ \KwTo $|\mathcal{W}|$}{
  ...
  \ForEach{$(l,k) \in \mathcal{R}$}{
    ...
    \tcp{Vectorized reconstruction of Eq.~\eqref{eq:reconstructed_cosine}}
    $\hat{\Gamma} \gets \widetilde{D}_{ij} \oslash \bigl(\sqrt{\widetilde{D}_{ii}}\odot \sqrt{\widetilde{D}_{jj}} + \varepsilon\bigr)$\;
    $n \gets \|\hat{\Gamma}\|_2 + \varepsilon$\;
    \tcp{Score of Eq.~\eqref{eq:rho_objective}}
